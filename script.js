...
const operate = function (num1, num2, operator) {
  switch (operator) {
    ...
    case "**":
      return toPower(Number(num1), Number(num2));
  }
};
...
for (let i = 0; i < buttons.length; i++) {
  ...
  if (buttons[i].classList.contains("number")) {
    ...
    buttons[i].addEventListener("click", (event) => {
      ...
      if (isTyping) {
        if (checkLength(mainText) > 1)
          mainScreen.textContent = mainText.slice(0, mainText.length - 1);
  ...
  } else if (buttons[i].classList.contains("evaluate")) {
    buttons[i].addEventListener("click", (event) => {
      if (firstNumber != null) {
        secondNumber = Number(mainScreen.textContent);
        let result = operate(firstNumber, secondNumber, lastOperator);
        ...
        mainScreen.textContent = betterResult(Number(mainScreen.textContent));
    });
  }
}
